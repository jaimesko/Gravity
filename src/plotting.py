...
class Plots:
    """Contains methods for 3 different plots along with an auxiliary path length limiting method."""
    ...
    def plot_2d(self, figsize: tuple = (8,8), trail_len: int = 30, aus: int = 6) -> None:
        ...
        def update(frame: int) -> list:
            #for i, point, line, text in zip(self.system, points, lines, texts):
            texts = []
            for i, point, line in zip(self.system, points, lines):
                ...
                #text.set_position(i.path[frame, :2])
        ...
        #texts = [ax.text([], [], i.name) for i in self.system]#ax.text(i.path[0, 0], i.path[0, 1]
